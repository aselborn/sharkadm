...
/**
 * Import format for ZooplanktonBiomad.
 * 
 */
public class FormatFileZooplanktonBiomad extends FormatFileBase {

	public FormatFileZooplanktonBiomad(PrintStream logInfo, FileImportInfo importInfo) {
...
//	}				
	...
	private void importIndwetwt(List<String[]> fileContent) {
		String[] header = null;
		if (ImportFacade.instance().isShowInfo()) {
			logInfo.println("Info: Indwetwt file: " + fileContent.size() + " rows (header included).");
		}
		int addedItems = 0;

		// Read all rows in the file indwetwt.skv. For each row check all variables in the dataset
		...
		for (String[] row : fileContent) {
			if (header == null) {
				...
				tmpLATNM = getCell(row, "LATNM");
				tmpSFLAG = getCell(row, "SFLAG");
				tmpSEXCO = getCell(row, "SEXCO");
				tmpSTAGE = getCell(row, "STAGE");
				tmpQFLAG = getCell(row, "QFLAG");
				tmpINDWETWT = getCell(row, "INDWETWT").replace(",", ".");

				if (tmpQFLAG.equals("<")) {
					
					// Don't use values if "under detection limit".
					
				} else {
					for (Visit tmpVisit : dataset.getVisits()) {
						for (Sample tmpSample : tmpVisit.getSamples()) {
							for (Variable tmpVariable : tmpSample.getVariables()) {
								if (
//									(tmpVariable.getField("variable.parameter").equals("MODIFY_VARIABLE.COUNTNR") || 
//									 tmpVariable.getField("variable.parameter").equals("INDCOUNT")) &&
									tmpVariable.getField("variable.reported_scientific_name").equals(tmpLATNM) &&
									tmpVariable.getField("variable.species_flag_code").equals(tmpSFLAG) &&
									tmpVariable.getField("variable.sex_code").equals(tmpSEXCO) &&
									tmpVariable.getField("variable.dev_stage_code").equals(tmpSTAGE)
																												) {
									
//									tmpVariable.addField("variable.COPY_VARIABLE_MULTIPLY.INDWETWT.mg/ind.1000", tmpINDWETWT);

									
									Double indwetwt = ConvUtils.convStringToDouble(tmpINDWETWT) * 1000;
									tmpVariable.addField("variable.used_indwetwt", indwetwt.toString()); // NOTE: unit ug.
									addedItems++;
		...
		if (ImportFacade.instance().isShowInfo()) {
			logInfo.println("Info: Indwetwt file, processed rows: " + addedItems + ".");
		}
	}				
