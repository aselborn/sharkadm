...
/**
 * Import format for PrimaryProduction.
 * 
 */
public class FormatFilePrimaryProduction extends FormatFileBase {

	public FormatFilePrimaryProduction(PrintStream logInfo, FileImportInfo importInfo) {
	...
	public void importFiles(String zipFileName, Dataset dataset) {
		...
		loadKeyTranslator(importMatrixColumn, "import_matrix_primaryproduction.txt");
...
//				utils.convDate(visit.getField("visit.visit_date")));
	...
	public void postReorganizeSample(Sample sample) {
		// SDEPT may contain for example "0-20".
		if (sample.getField("sample.sample_depth_m").contains("-")) {
//			String parts[] = sample.getField("sample.sample_depth_m").split("-");
			String parts[] = sample.getField("sample.sample_depth_m").split(Pattern.quote("-"));
			sample.addField("sample.sample_min_depth_m", parts[0].trim());
			sample.addField("sample.sample_max_depth_m", parts[1].trim());
		}
	}
