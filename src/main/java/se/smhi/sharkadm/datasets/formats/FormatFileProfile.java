...
/**
 * Import format for Profile.
 * 
 */
public class FormatFileProfile extends FormatFileBase {

	public FormatFileProfile(PrintStream logInfo, FileImportInfo importInfo) {
	...
	public void importFiles(String zipFileName, Dataset dataset) {
		...
		loadKeyTranslator(importMatrixColumn, "import_matrix_profile.txt");
		...
		try {
			if (Files.exists(Paths.get(zipFileName, "processed_data", "metadata.txt"))) {
	...
	private void importData(List<String[]> fileContent) {
		...
		for (String[] row : fileContent) {
			if (header == null) {
				...
				// ADD DUMMY VARIABLE.
				currentVariable.setParameter("CTD Profile");
				currentVariable.setValue(getCell(row, "FILE_NAME"));
				currentVariable.setUnit("file name");
...
//				   getCellByKey(row, "visit.reported_station_name");
		String keyString = getCellByKey(row, "visit.visit_year") + ":" +
